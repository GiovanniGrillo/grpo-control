...
class CGRPO:
    ...
    def update(self):        
        ...
        pca_clustering = PCA(n_components=pca_comps) # Keep enough components to retain most variance for clustering
        ...
        # jitter = np.random.normal(0, 1e-6, all_states_pca.shape)
        # all_states_pca += jitter

        # DBSCAN parameters
        total_points = all_states_pca.shape[0]
        dynamic_cluster_size = int(max(50, total_points * 0.01))
        dynamic_min_samples = int(max(10, dynamic_cluster_size / 5)) # 10 & 5
        ...
        for i in range(self.N):
            ...
            if self.lam_d > 0:                                                                          # Only calculate diversity regularization if lambda_d is greater than 0
                mu_i_avg = dist.mean.mean(dim=0)                                             # Mean action of the current policy i (used for diversity regularization)
                for j in range(self.N):
                    if i == j:
                        continue
                    mu_j_avg = all_mus[j].mean(dim=0)                                                                   # Mean action of policy j
                    cos_sim = torch.cosine_similarity(mu_i_avg.view(1, -1), mu_j_avg.view(1, -1), dim=-1)                      # Calculate cosine similarity between the mean actions of policy i and policy j
                    reg_term = torch.clamp(cos_sim - self.tau, min=0.0)                                                           # Calculate the regularization term based on cosine similarity and the threshold tau
                    l_diversity += reg_term.mean()     # Diversity regularization to encourage different policies to explore different strategies (penalize high similarity above a threshold tau)
            l_diversity = l_diversity / (self.N * (self.N - 1))
