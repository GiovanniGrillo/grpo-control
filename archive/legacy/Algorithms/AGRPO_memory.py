...
class AGRPO:
    """
    Advanced Group Relative Policy Optimization (AGRPO) with TRAUMA MEMORY.
    
    Features:
    - Persistent replay buffer for failed episodes
    - Inverse RL loss term to avoid traumatic action sequences
    - Trauma centers tracked in feature space
    """
    def __init__(self, env, seed=42, hidden_dim=256, lr=5e-4, N=50, K=2, epsilon=0.2,
        ...
        # Trauma replay buffer for persistent memory
        self.trauma_replay_buffer = bf.TraumaReplayBuffer(max_size=5000, worst_percentile=15)
        ...
        for i in range(len(self.actors)):
            ...
            if traj_return < self.dynamic_threshold:
                ...
                if len(trauma_points) > 2:
                    ...
                    # Add to trauma replay buffer for future learning
                    traj = self.buffer.get_latest_trajectory(i)
                    self.trauma_replay_buffer.add(
                        obs_list=traj["obs"],
                        action_list=traj["action"],
                        feature_list=traj["feature"],
                        reward_list=traj["reward"],
    ...
    def update(self):
        ...
        for i in range(len(self.actors)):
            ...
            # Trauma replay loss: learn to avoid actions from failed episodes
            l_replay = torch.tensor(0.0).to(self.device)
            if len(self.trauma_replay_buffer.buffer) > 32:
                replay_batch = self.trauma_replay_buffer.sample(32)
                replay_weight = 0.05  # Small weight for replay term
                
                for sample in replay_batch:
                    try:
                        # Get features from replay trajectory
                        replay_obs = [torch.FloatTensor(o).to(self.device) if not isinstance(o, torch.Tensor) else o.to(self.device) for o in sample['obs']]
                        replay_actions = [torch.FloatTensor(a).to(self.device) if not isinstance(a, torch.Tensor) else a.to(self.device) for a in sample['action']]
                        
                        if len(replay_obs) > 0 and len(replay_actions) > 0:
                            replay_obs_t = torch.stack(replay_obs)
                            replay_actions_t = torch.stack(replay_actions)
                            
                            # Forward through policy
                            feat_replay = self.actors[i].forward_features(replay_obs_t)
                            dist_replay = self.actors[i].get_distribution(feat_replay)
                            log_prob_replay = dist_replay.log_prob(replay_actions_t).sum(dim=-1)
                            
                            # Loss: minimize log-prob of traumatic actions (inverse RL principle)
                            l_replay += -log_prob_replay.mean() * replay_weight
                    except:
                        pass  # Skip malformed samples
                
                if isinstance(l_replay, torch.Tensor) and l_replay.item() != 0:
                    l_replay = l_replay / max(1, len(replay_batch))

            total_loss = actor_loss + (self.lam_s * l_smooth) + l_div + l_trauma + l_replay
        ...
        replay_stats = self.trauma_replay_buffer.get_stats()
        stats_dict = {
            ...
            "trauma_replay_buffer_size": replay_stats['size'],
            "trauma_replay_min_return": replay_stats['min_return'],
            "trauma_replay_mean_return": replay_stats['mean_return'],
